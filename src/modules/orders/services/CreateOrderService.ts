...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductsWithNoQuantityAvaliable = products.filter(
      product => existentProducts.filter(prod => prod.id === product.id)[0].quantity < product.quantity
    );

    if (findProductsWithNoQuantityAvaliable.length) {
      throw new AppError(`The quantity ${findProductsWithNoQuantityAvaliable[0].quantity} is not avaliable for ${findProductsWithNoQuantityAvaliable[0].id}`)
    }
